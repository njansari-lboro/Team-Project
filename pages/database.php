...
function get_record($table = false, $id = false, $keyfield = "id", $override_da = false)
...
//apply data access permissions too
{
	...
	if (!array_key_exists('business_id', $params) && array_key_exists($table, $GLOBALS['business_id_tables']) && $GLOBALS['userbizid']) {
		$where[] = "business_id = " . $GLOBALS['userbizid'];
	}
...
function check_data_access($table = false, $id = false, $op = "read")
{
	...
	$rec = get_record($table, $id, 'id', true);
	if (!$rec) {
		return false;
	}
